  ...
  return (
    <div className="space-y-12">
      <Suspense fallback={
        ...
        <DashboardDataWrapper session={session} tenantId={tenantId} />
      </Suspense>
    </div>
  );
}

async function DashboardDataWrapper({ session, tenantId }: { session: any, tenantId: string }) {
  // Fetch Tenant Branding for the title/subtitle
  ...
  const [
    ...
    dbGalleries, dbClients, dbBookings, dbAgents, dbServices, dbTeamMembers, allGalleries, isSubscribed
  ] = await Promise.all([
    ...
    prisma.gallery.findMany({ where: galleryWhere, orderBy: { createdAt: 'desc' }, take: 8, include: { client: { select: { id: true, name: true, businessName: true } }, property: { select: { id: true, name: true } }, media: { select: { thumbnailUrl: true, url: true }, take: 10 }, services: { include: { service: true } }, favorites: { select: { id: true } }, booking: { include: { assignments: { include: { teamMember: { select: { displayName: true } } } } } } } }),
    prisma.client.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, name: true, businessName: true, avatarUrl: true } }),
    prisma.booking.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, title: true, clientId: true, property: { select: { name: true } }, services: { include: { service: true } } } }),
    prisma.agent.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, name: true, clientId: true, avatarUrl: true } }),
    prisma.service.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, name: true, price: true, icon: true } }),
    prisma.teamMember.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, displayName: true, avatarUrl: true } }),
    prisma.gallery.findMany({ where: { tenantId, deletedAt: null }, select: { id: true, title: true } }),
